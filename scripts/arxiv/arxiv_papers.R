...
file_subfield_counts <- here::here("data/arxiv/subfield_counts.rds")
  # Output file
file_out <- here::here("data/arxiv/arxiv_papers.rds")
...
papers %>%
  ...
  distinct(title, .keep_all = TRUE) %>% # There are some duplicate titles. Most of these appear to be the same paper, listed multiple times
